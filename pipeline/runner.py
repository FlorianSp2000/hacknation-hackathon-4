...
from core.types import BBox, FrameResult, Mask, SegmentationResult
    ...
    def _run_segmenter_on_detections(
        ...
        try:
            for det_idx, box in enumerate(boxes):
                ...
                iou_to_refresh = (
                    _bbox_iou(current_bbox, track.refresh_bbox)
                    if track.refresh_bbox is not None
                    else 0.0
                )
                should_refresh = (
                ...
                if self.sequential_offload:
                    ...
                    if self.segmenter:
                        ...
                        if guided_seg and result.detection is not None:
                            ...
                            seg_future = executor.submit(self._run_segmenter, frame, idx)
                            result.segmentation = seg_future.result()
                        result.timings["segmenter"] = perf_counter() - t0
