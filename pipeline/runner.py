...
from core.base import Detector, Segmenter, StateClassifier, Tracker, VLM
...
class Pipeline:
    ...
    def _run_state_classifier(self, frame, tracking_result, idx):
        ...
        boxes = [(b.x1, b.y1, b.x2, b.y2, b.track_id) for b in tracking_result.boxes]
            ...
            for idx, frame in read_frames(video_path, max_frames):
                ...
                if self.sequential_offload:
                    ...
                    if self.state_classifier and result.tracking:
                        t0 = perf_counter()
                        result.state_classification = self._run_state_classifier(proc_frame, result.tracking, idx)
